...
// 函数：填充下拉列表
function populateDropDown(list, defaultCurrency) {
  for (let currency_code in country_list) {
    let selected = currency_code === defaultCurrency ? "selected" : "";
...
dropList.forEach((list, index) => {
  populateDropDown(list, index === 0 ? "CNY" : "USD");
  list.addEventListener("change", (e) => loadFlag(e.target));
});
